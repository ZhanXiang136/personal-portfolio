...
const projects = [
  {
    title: 'Real-Time Messaging Platform', date: 'Jan — May 2026', kind: 'Distributed systems',
  ...
  {
    title: 'Stock Trading AI App', date: 'Jun 2025 — May 2026', kind: 'Applied machine learning',
  ...
  {
    title: 'Tic-Tac-Toe AI', date: 'Python · Flask', kind: 'Minimax game AI', url: 'https://github.com/ZhanXiang136/tictactoe-ai', game: true,
...
const earlierProjects = [
  { title: 'Pokemon Battle Simulator', tags: ['Java', 'Game systems', 'UI'], image: PokemonMenu, preview: PokemonBattle, url: 'https://github.com/ZhanXiang136/PokemonGame' },
  { title: 'Infinite Maze Runner', tags: ['JavaScript', 'Procedural play', 'Team build'], image: Maze, preview: MazeSample, url: 'https://github.com/dhuang6334/Code-Overflow' },
];
...
function EarlierProject({ project, index }) {
  ...
  return <FadeComponent direction={index ? 'left' : 'right'}>
    <motion.a ref={ref} className="project-card" href={project.url} target="_blank" rel="noreferrer"
      ...
      <div className="project-meta"><div><h3>{project.title}</h3><p>{project.tags.join(' / ')}</p></div><b>↗</b></div>
    </motion.a>
  </FadeComponent>;
}
  ...
  return <div className="project-section">{projects.map((project, index) =>
    <FadeComponent direction={index ? 'left' : 'right'} key={project.title}>
      <motion.article className={`project-card project-feature project-feature-${index + 1}`} whileHover={{ y: -4 }}>
        ...
        <div className="project-meta"><div><h3>{project.title} {project.url && <a className="project-repo-link" href={project.url} target="_blank" rel="noreferrer" aria-label={`Open ${project.title} repository`}>↗</a>}</h3><p>{project.date}</p></div></div>
